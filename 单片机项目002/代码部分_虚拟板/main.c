/********************************************************************************
* 单片机综合项目 - 桌面多功能智能摆件全联调优化版 main.c（Proteus 虚拟仿真版）
*
* 优化特性：
* 1. 彻底解决 Proteus 仿真中计算器第一行 LCD1602 汉字显示为日文乱码的问题。
* 2. 重构计算器（选项四）显示逻辑：输入数字时普通整数实时刷新，杜绝画面卡死在0.01；
* 按下运算符或等号时自动对齐伪浮点百倍基准线，结果精准保留两位小数。
* 3. 完美集成全套实用功能：开机密码锁(123)、K2长按1秒无缝安全退出、
* 温度>40℃蜂鸣器长鸣报警、流式串口字符显示及系统精准运行计时。
* 4. 【手动切页优化】：彻底移除主菜单的自动轮播，改由独立按键 K3 (左切) 与 K4 (右切) 
...
void main(void){    
    ...
    // 50次高频盲扫，洗净 Proteus 上电瞬间准双向口产生的漏电假键值    
    for (i = 0; i < 50; i++) {        
        delay_ms(5);
...
static void pwd_input_service(void){    
    ...
    if (calc_is_digit(matrix_key) && pwd_idx < 3) {        
        ...
        LCD1602_display_char(2, 7 + pwd_idx, '*'); // 压下瞬间直接丝滑打出掩码        
        pwd_idx++;
        if (pwd_idx == 3) {            
            delay_ms(400);
            if (pwd_buffer[0] == 1 && pwd_buffer[1] == 2 && pwd_buffer[2] == 3) {                
                system_state = STATE_MENU;
                reset_runtime(); // 密码验证通过，后台秒表此时开始精确运行计时
            } else {                
...
/* 选项四：支持两位小数的定点数计算器（彻底解决仿真不刷新与乱码问题） */
static void calc_service(void){    
    u8 d;   
    if (first_flag) {        
        first_flag = 0;        
    ...
    if (matrix_key == MATRIX_S13) {        
        if (calc_state == CALC_INPUT_FIRST || calc_state == CALC_INPUT_SECOND) {            
            has_dot = 1;           
            dot_cnt = 0;            
            LCD1602_display_char(2, 15, '.'); // 在屏幕右下角点亮一个点，给予用户视觉反馈       
        }        
        return;   
    }    
    switch (calc_state)    
